...
def heuristica(fila, colum):
    # Calcula la distancia de Manhattan al objetivo
    return abs(fila - fila_final) + abs(colum - columna_final)
...
def busqueda_avara():
    ...
    cola = PriorityQueue()
    # Inicializar la búsqueda avara desde la posición inicial con costo 0
    nodo_inicial = Nodo(fila_inicio, columna_inicio, 0, None)
    cola.put((heuristica(fila_inicio, columna_inicio), nodo_inicial))
    ...
    while not cola.empty():
        ...
        # Expansión de vecinos basada en la heurística
        for df, dc in [(-1, 0), (0, 1), (1, 0), (0, -1)]:
            ...
            if es_valido(nueva_fila, nueva_colum) and valido:
                ...
                cola.put((heuristica(nueva_fila, nueva_colum), nuevo_nodo))
