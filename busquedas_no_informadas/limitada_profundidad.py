...
def busqueda_Limitaprofundidad(tablero, lista_nodos_iniciales, meta, maximo_iteraciones,graph):
    ...
    while pila:
        ...
        if nodo_actual.fila == fila_final and nodo_actual.columna == columna_final:
            camino = reconstruir_camino(nodo_actual)  # Reconstruir el ca   mino
            graph.graficar_arbol(nodo_actual, camino)
            return (True, camino)

        hijos_temp = []
        for movimiento in [(-1, 0), (0, 1), (1, 0), (0, -1)]: # Movimientos posibles abajo, izquierda, arriba, derecha
